      ...
      $("#pageMarker_draggable").toggle(function(){
        if(draggable.style.display=="none") {
          ...
          chrome.runtime.sendMessage({from: "content_scripts"}, function(response) {
            ...
            let fileDate = date.getFullYear() + "-" + ("0" + (date.getMonth() + 1)).slice(-2).toString() + "-" + ("0" + date.getDate()).slice(-2)+" "+ date.getHours() + "-" + date.getMinutes()+ "-" + date.getSeconds();
            var e = document.createElement("a");
            e.download = "Screenshot from " + fileDate + " Web Draw.png", e.href = screenshot, document.body.appendChild(e), e.click(),
            document.body.removeChild(e);
            var img = '<img style="-webkit-user-select: none;margin: auto;cursor: zoom-in;" width="100%" src="' + screenshot + '"></img>' 
